...
public class DispatchService {
    ...
    public DispatchResponse generateDispatchPlan() {
        ...
        // Store the original position of each vehicle to calculate the complete route distance later
        Map<String, double[]> startPositions = new HashMap<>();
        for (Vehicle v : vehicles) {
            startPositions.put(v.getVehicleId(),
                    new double[]{ v.getCurrentLatitude(), v.getCurrentLongitude() });
        }

        // Sort orders by priority so that HIGH priority orders are assigned first
        List<DeliveryOrder> sortedOrders = deliveryOrders.stream()
                .sorted(Comparator.comparing(this::priorityValue))
                .toList();

        // Stores the IDs of orders that cannot be assigned to any vehicle
        List<String> unassignedOrderIds = new ArrayList<>();

        // Assign each order to the nearest vehicle with sufficient remaining capacity
        for (DeliveryOrder order : sortedOrders) {
            ...
            if (bestVehicle != null) {
                ...
                unassignedOrderIds.add(order.getOrderId());
            }
        }
        ...
        for (Vehicle v : vehicles) {
            ...
            double[] start = startPositions.get(v.getVehicleId());
            double prevLat = start[0];
            double prevLon = start[1];
        ...
        return new DispatchResponse(result, unassignedOrderIds);
    }
