...
class DispatchServiceTest {
    ...
    void shouldAssignOrderToVehicle() {
        ...
        DispatchResponse result = service.generateDispatchPlan();

        assertEquals(1, result.getDispatchPlan().get(0).getAssignedOrders().size());
    }
    ...
    void shouldNotAssignIfCapacityExceeded() {
        ...
        assertEquals(0, result.getDispatchPlan().get(0).getAssignedOrders().size());
    }
    ...
    void shouldAssignHighPriorityFirst() {
        ...
        assertEquals("HIGH", result.getDispatchPlan().get(0).getAssignedOrders().get(0).getPriority());
    }
    ...
    void shouldAssignToNearestVehicle() {
        ...
        VehiclePlan assigned = result.getDispatchPlan().stream()
    ...
    @Test
    void shouldUpdateVehicleLocationInMemoryDuringCalculation() {
        DeliveryOrder order = createOrder("ORD1", 10, "HIGH", 28.6139, 77.2090);
    ...
    @Test
    void shouldNotPersistVehicleChangesToDatabase() {
        DeliveryOrder order = createOrder("ORD1", 10, "HIGH", 28.6139, 77.2090);
        Vehicle vehicle = createVehicle("VEH1", 50, 28.7041, 77.1025);
    ...
    void shouldCalculateDistance() {
        ...
        String distance = result.getDispatchPlan().get(0).getTotalDistance();
        assertTrue(distance.contains("km"));
    }

    @Test
    void shouldReportUnassignedOrderWhenNoVehicleHasCapacity() {
        DeliveryOrder tooHeavy = createOrder("ORD1", 200, "HIGH", 28.6139, 77.2090);
        Vehicle vehicle = createVehicle("VEH1", 50, 28.6139, 77.2090);

        when(orderRepository.findAll()).thenReturn(List.of(tooHeavy));
        when(vehicleRepository.findAll()).thenReturn(new ArrayList<>(List.of(vehicle)));

        DispatchResponse result = service.generateDispatchPlan();

        assertEquals(List.of("ORD1"), result.getUnassignedOrderIds());
        assertEquals(0, result.getDispatchPlan().get(0).getAssignedOrders().size());
    }
}